...
def build(base:Path,extension_root:Path,out:Path):
    ...
    er_bytes=(extension_root/'report.json').read_bytes();er=json.loads(er_bytes)
    ...
    known={(r['season'],r['gw']) for r in rows};additions=[]
    for witness in json.loads(checked(extension_root/'witnesses.json',er['witnesses_sha256'])):
        key=(witness['season'],witness['gw'])
        if key in known:raise ValueError('duplicate selected deadline')
        c=candidates[key];h=witness['archive_hour']
        hour=json.loads(checked(extension_root/'hours'/(h+'.json'),er['hour_report_sha256'][h]));extension.validate(extension_root,hour)
    ...
    result=dict(version='calendar-extension-selection-v1',parent_report_sha256=digest(parent_bytes),extension_report_sha256=digest(er_bytes),
...
def main():
    ...
    for name in ('base-root','extension-root','out'):p.add_argument('--'+name,type=Path,required=True)
