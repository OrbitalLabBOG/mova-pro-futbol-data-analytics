...
def verify(package: Path) -> dict:
    ...
    for entry in manifest['partitions']:
        ...
        if manifest.get('version') in ('fpl-labels-v5','fpl-labels-v6','fpl-labels-v7','fpl-labels-v8') and not frame.entity_type.eq('player').all():raise ValueError('non-player in player partition')
        rows+=len(frame)
