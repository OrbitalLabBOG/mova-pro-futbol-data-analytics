...
REVISION='34f681c7b756ae901982e53cdcb6cd061dbd0482'


def inventory(log_bytes):
    history=parse_log(log_bytes.decode());versions=[];excluded=[]
    for path,changes in sorted(history.items()):
        if not re.fullmatch(r'data/(?:fixtures\.csv|(?:fixtures_snapshots|snapshots)/fixtures_[0-9_-]+\.csv|raw/[0-9-]+/fixtures\.json)',path):continue
        for change in changes:
            ...
            versions.append(dict(path=path,season='unverified',**change))
...
def build(log:Path,out:Path):
    ...
    report=dict(version='additional-fixture-history-v1',repository=REPOSITORY,pinned_revision=REVISION,
