...
from mova_fpl.models.registry import ROOT, git_sha
...
def load_snapshot(path: Path) -> tuple[dict, list, dict]:
    """Carga un snapshot solo si ambos payloads conservan su SHA-256."""
...
def collect(season: str, gw: int, out_root: Path) -> tuple[Path, dict]:
    ...
    boot_raw, fixtures_raw = fetch_bootstrap(), fetch_fixtures()
