...
| C-04 | Q-02 sin respuesta cambia la función objetivo de REQ-F-006 (maximizar puntos vs rank ajustado por ownership) | ~~Bloqueo para WP-006~~ → **resuelto por diseño (ADR-007, 2026-08-07)**: v1 maximiza puntos esperados y el caso mini-liga es un término lineal adicional (`risk_lambda = 0`) sobre las mismas variables. Cambiar de objetivo es configuración, no refactor | Julián |
