...
def test_shadow_migration_is_versioned_and_contains_required_schemas() -> None:
    ...
    research_sql = (MIGRATIONS / "017_research_evidence_coverage.sql").read_text(
        encoding="utf-8"
    ).lower()
    assert "research.runs" in research_sql
    assert "research.documents" in research_sql
    rehearsal_sql = (MIGRATIONS / "018_browser_rehearsals.sql").read_text(
        encoding="utf-8"
    ).lower()
    ...
    worker_attempt_sql = (MIGRATIONS / "022_agent_worker_attempt_ledger.sql").read_text(
        encoding="utf-8"
    ).lower()
    assert "agent.worker_attempt_events" in worker_attempt_sql
    assert "unique (attempt_id, event_type)" in worker_attempt_sql
    accounting_sql = (MIGRATIONS / "023_physical_attempt_budget_accounting.sql").read_text(
        encoding="utf-8"
    ).lower()
    ...
    assert "create role mova_app_runtime login" in role_sql
    assert "create role mova_readonly_runtime login" in role_sql
    assert "default_transaction_read_only = on" in role_sql
    assert "revoke temporary" in role_sql
    assert "research.conflicts" in research_sql
